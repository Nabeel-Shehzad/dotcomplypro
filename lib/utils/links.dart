class Links{
  //add static String variable
  static String login = "https://dotcomplypro.com/MobileScripts/login.php";
  static String register = "https://dotcomplypro.com/MobileScripts/register.php";
  static String drug = "https://dotcomplypro.com/MobileScripts/drug.php";
  static String vehicle = "https://dotcomplypro.com/MobileScripts/vehicle.php";
  static String boc = "https://dotcomplypro.com/MobileScripts/boc.php";
  static String eca = "https://dotcomplypro.com/MobileScripts/eca.php";
  static String ucr = "https://dotcomplypro.com/MobileScripts/ucr.php";
  static String driver = "https://dotcomplypro.com/MobileScripts/driver.php";
  static String notification = "https://dotcomplypro.com/MobileScripts/notifications.php";
  static String send_token = "https://dotcomplypro.com/MobileScripts/send_token.php";
  static String get_driver = "https://dotcomplypro.com/MobileScripts/get_driver.php";
  static String files_url = "http://dotcomplypro.com/MobileScripts/";
  static String get_notify = "https://dotcomplypro.com/MobileScripts/get_notify.php";
  static String payment = "https://dotcomplypro.com/MobileScripts/payment.php";
  static String get_payment = "https://dotcomplypro.com/MobileScripts/get_payment.php";
  static String get_boc_doc = "https://dotcomplypro.com/MobileScripts/get_boc_doc.php";
  static String get_ucr_doc = "https://dotcomplypro.com/MobileScripts/get_ucr_doc.php";
}